...
public class TZDeviceListActivity extends AppCompatActivity {
    ...
    public void selectTZDevice(View view) {
        ...
        if (view.getId() == R.id.tzLefu1) {
            intent.putExtra("tzDeviceName", "AS-CF/CW BT");
            intent.putExtra("tzDeviceModel", TzUtil.TZ_DEVICE_LEFU_BT);
        } else if (view.getId() == R.id.tzLefu2) {
            intent.putExtra("tzDeviceName", "AS-CF/CW BLE");
            intent.putExtra("tzDeviceModel", TzUtil.TZ_DEVICE_LEFU_BLE);
