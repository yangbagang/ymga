 ...
 */
@SuppressLint("NewApi")
public class TZMain2Activity extends SubActivity {
        ...
        if (getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)) {
            ...
            if (TzUtil.TZ_DEVICE_LEFU_BLE.equalsIgnoreCase(tzDevice)) {//体脂秤lefu
                if (lefuService != null) {
                    ...
                    getApplicationContext().bindService(new Intent(TZMain2Activity.this, LefuService.class),
                            lefuConnection, Context.BIND_AUTO_CREATE);
                }
            } else if (TzUtil.TZ_DEVICE_LEFU_BT.equalsIgnoreCase(tzDevice)) {
                if ("启动".equals(action)) {
                    // 还未绑定，开始绑定过程
                    PollingUtils.startPollingService(TZMain2Activity.this, 10,
                            ScaneBluetoothService.class, ScaneBluetoothService.ACTION);
                    tzPJOperator.setText("停止");
                } else {
                    PollingUtils.stopPollingService(TZMain2Activity.this,
                            ScaneBluetoothService.class, ScaneBluetoothService.ACTION);
                ...
                if (MyApplication.bleService != null) {
                    ...
                    getApplicationContext().bindService(new Intent(TZMain2Activity.this, BluetoothService.class), furuikConnection, Context.BIND_AUTO_CREATE);
                }
            }
        } else {
            ybgApp.showMessage(getApplicationContext(), "您的手机当前不支持蓝牙4.0，无法连接体指秤。");
    ...
    protected void onStart() {
        ...
        bindIntent = new Intent(TZMain2Activity.this, TZDataService.class);
